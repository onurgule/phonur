...
var pOK = false;
	if (Object.keys(iso3166).length === 0) {
	 ...
	 if (plus_sign) {
			...
			if(!isNaN(iso3166.country_code)) {pOK = true; console.log("1",iso3166.country_code,iso3166.alpha2);}
			
			
		} 
		else {
				iso3166 = get_iso3166_by_phone("+"+formatPhone);
			if(iso3166!={}) {pOK = true; console.log("2",iso3166);}
			}
	if (formatCountry && !pOK) {
	...
	if (validate_phone_iso3166(formatPhone, iso3166)) {
		return ['+' + formatPhone, iso3166.country_code, iso3166.alpha2];
	}
	return result;
};
